...
public class Myad extends RecyclerView.Adapter<Myad.itemViewholder>{

    Context context;
    ArrayList<OrdersHelper> list;


    public Myad(Context context, ArrayList<OrdersHelper> list) {
    ...
    @NonNull
    @Override
    public Myad.itemViewholder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        return new Myad.itemViewholder(LayoutInflater.from(context).inflate(R.layout.currentorders,parent,false));
    }

    @Override
    public void onBindViewHolder(@NonNull Myad.itemViewholder holder, int position) {
            ...
            ref.addListenerForSingleValueEvent(new ValueEventListener() {
                ...
                public void onDataChange(@NonNull DataSnapshot snapshot) {
                    for (DataSnapshot dataSnapshot : snapshot.getChildren()) {
                        OrdersHelper ordersHelper = dataSnapshot.getValue(OrdersHelper.class);
                        if (ordersHelper.getOrderID().equals(itemClass.getOrderID())) {
                            ref.child(ordersHelper.getOrderID()).child("status").setValue("completed");
                            Intent intent1=new Intent(context,AdminNewOrdersActivity.class);
